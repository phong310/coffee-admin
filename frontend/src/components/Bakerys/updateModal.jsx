...
const UpdateModal = ({ data, setData, getAll, item }) => {
    ...
    const handleUpdate = () => {
        form.validateFields().then(async () => {
            ...
            try {
                await axios.put(`http://localhost:7000/bakery/update/${item?._id}`, newUpdate)
                setData(false);
                toast.success("Cập nhật bánh thành công");
                reset_form();
                getAll()

            } catch (e) {
                console.log("Err:", e);
                toast.warning("Cập nhật bánh thất bại !")
            }
        })
