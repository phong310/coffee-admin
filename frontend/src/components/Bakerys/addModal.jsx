...
const AddModal = ({ data, setData, getAll }) => {
    ...
    const createNewDrinks = () => {
        formRef.current.validateFields().then(async () => {
            ...
            try {
                const response2 = await axios.post('http://localhost:7000/bakery/addNewBakery', newBakerys);
                console.log(response2)
                setData(false);
                toast.success("Thêm mới bánh thành công");
                reset_form()
                getAll();
            } catch (e) {
                console.log("Lỗi rồi:", e)
                toast.warning("Thêm mới bánh thất bại !");
            }
        })
    }
